...
const FALLBACK_SCROLL_SPEED = 24; // seconds, only used if the admin value is missing/invalid
...
export function AnnouncementStrip() {
  ...
  useEffect(() => {
    getAnnouncement().then((data) => {
      if (!data || !data.enabled || data.announcements.length === 0) return;
    ...
    <div className="relative w-full bg-canvas border-b border-hairline overflow-hidden">
      ...
      <div className="flex items-center h-9 max-w-screen-2xl mx-auto">
        ...
        <button
          ...
          className="flex-none px-4 h-full flex items-center text-ink-subtle hover:text-ink transition-colors"
        >
...
function RotateStrip({
  ...
  // Index can briefly point past the end if a new (shorter) announcement
  // list loads while rotated further in — clamp defensively.
